        ...
        (async () => {
            try {
                ...
                if (!html.includes("/logout")) {
                    throw new Error("Not logged in to Codeforces");
                }
                // Extract Handle (Look for header link next to Logout)
                // Pattern: <a href="/profile/HANDLE">HANDLE</a> | <a href=".../logout">Logout</a>
                const match = html.match(/href="\/profile\/([^"]+)"[^>]*>[^<]+<\/a>\s*\|\s*<a[^>]*logout/i);

                if (match && match[1]) {
                    sendResponse({ success: true, handle: match[1] });
                } else {
                    throw new Error("Could not find handle in header (Login check passed but parsing failed)");
                }
            } catch (err) {
                sendResponse({ success: false, error: err.message });
            }
